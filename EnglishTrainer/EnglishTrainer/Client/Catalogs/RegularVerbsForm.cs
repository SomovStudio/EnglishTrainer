...
{
	/// <summary>
	/// Description of RegularVerbsForm.
	/// </summary>
	public partial class RegularVerbsForm : Form
	{
		public RegularVerbsForm()
		{
			//
		...
		void RegularVerbsFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM RegularVerbs ORDER BY V1_Infivitive ASC";
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "RegularVerbs";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM RegularVerbs WHERE (Verb_Translation LIKE '%" + toolStripComboBox1.Text + 
		...
		void returnValue()
		{
			if(listView1.SelectedIndices.Count > 0){
				if(ChangeVerbType == Constants.VERB_PRESENT_PARTICIPLE)
				{
					TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[4].Text;
				}
				else if(ChangeVerbType == Constants.VERB_PAST_SIMPLE)
				{
					TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[2].Text;
				}
				else if(ChangeVerbType == Constants.VERB_PAST_PARTICIPLE)
				{
					TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[3].Text;
				}
				else{
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_REGULAR_VERBS;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM RegularVerbs WHERE (ID = " + fileID + ")");
					if(query.Execute()){
