...
namespace EnglishTrainer.Client.Catalogs
{
	...
	public partial class ConditionsForm : Form
	{
		...
		void ConditionsFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Conditions ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Conditions")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["Condition_English"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["Condition_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "Conditions";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Conditions WHERE (Adverb_Translation LIKE '%" + toolStripComboBox1.Text + 
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_CONDITIONS;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM Conditions WHERE (ID = " + fileID + ")");
					if(query.Execute()){
