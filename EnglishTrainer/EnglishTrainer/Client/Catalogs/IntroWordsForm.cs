...
{
	/// <summary>
	/// Description of IntroWordsForm.
	/// </summary>
	public partial class IntroWordsForm : Form
	{
		public IntroWordsForm()
		{
			//
		...
		void IntroWordsFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM IntroductoryWords ORDER BY ID ASC";
			if(oleDb.ExecuteFill("IntroductoryWords")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["IntroWord_English"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["IntroWord_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "IntroductoryWords";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM IntroductoryWords WHERE (IntroWord_Translation LIKE '%" + toolStripComboBox1.Text + 
				"%' OR IntroWord_English LIKE '%" + toolStripComboBox1.Text + "%' ) ORDER BY ID ASC";
			if(oleDb.ExecuteFill("IntroductoryWords")){
				listView1.Items.Clear();
				ListViewItem ListViewItem_add;
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_INTRO_WORDS;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM IntroductoryWords WHERE (ID = " + fileID + ")");
					if(query.Execute()){
