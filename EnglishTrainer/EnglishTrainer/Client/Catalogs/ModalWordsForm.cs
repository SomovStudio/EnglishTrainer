...
{
	/// <summary>
	/// Description of ModalWordsForm.
	/// </summary>
	public partial class ModalWordsForm : Form
	{
		public ModalWordsForm()
		{
			//
		...
		void ModalWordsFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM ModalWords ORDER BY ID ASC";
			if(oleDb.ExecuteFill("ModalWords")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["ModalWord_English"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["ModalWord_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "ModalWords";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM ModalWords WHERE (ModalWord_Translation LIKE '%" + toolStripComboBox1.Text + 
				"%' OR ModalWord_English LIKE '%" + toolStripComboBox1.Text + "%' ) ORDER BY ID ASC";
			if(oleDb.ExecuteFill("ModalWords")){
				listView1.Items.Clear();
				ListViewItem ListViewItem_add;
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_MODAL_WORDS;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM ModalWords WHERE (ID = " + fileID + ")");
					if(query.Execute()){
