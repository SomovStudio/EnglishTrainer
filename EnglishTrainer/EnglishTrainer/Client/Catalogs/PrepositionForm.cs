...
{
	/// <summary>
	/// Description of PrepositionForm.
	/// </summary>
	public partial class PrepositionForm : Form
	{
		public PrepositionForm()
		{
			//
		...
		void PrepositionFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Preposition ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Preposition")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["Preposition_English"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["Preposition_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "Preposition";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Preposition WHERE (Preposition_Translation LIKE '%" + toolStripComboBox1.Text + 
				"%' OR Preposition_English LIKE '%" + toolStripComboBox1.Text + "%' ) ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Preposition")){
				listView1.Items.Clear();
				ListViewItem ListViewItem_add;
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_PREPOSITION;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM Preposition WHERE (ID = " + fileID + ")");
					if(query.Execute()){
