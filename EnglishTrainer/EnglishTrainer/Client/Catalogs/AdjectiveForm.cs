...
{
	/// <summary>
	/// Description of AdjectiveForm.
	/// </summary>
	public partial class AdjectiveForm : Form
	{
		...
		void AdjectiveFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Adjective ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Adjective")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["Positive"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["Comparative"].ToString());
					ListViewItem_add.SubItems.Add(rowElement["Superlative"].ToString());
					ListViewItem_add.SubItems.Add(rowElement["Adjective_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "Adjective";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Adjective WHERE (Adjective_Translation LIKE '%" + toolStripComboBox1.Text + 
				"%' OR Positive LIKE '%" + toolStripComboBox1.Text + 
				"%' OR Comparative LIKE '%" + toolStripComboBox1.Text +
				"%' OR Superlative LIKE '%" + toolStripComboBox1.Text +
				"%' ) ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Adjective")){
				listView1.Items.Clear();
				ListViewItem ListViewItem_add;
		...
		void ButtonReturnPositiveClick(object sender, EventArgs e)
		{
		...
		{
			if(listView1.SelectedIndices.Count > 0){
				TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[3].Text.ToString();
				Close();
			}
		}
		...
		public void ShowReturnValue()
		{
			...
			выбратьПоложительнуюСтепеньToolStripMenuItem.Visible = true;
			выбратьСравнительнуюСтепеньToolStripMenuItem.Visible = true;
			выбратьПревосходнуюСтепеньToolStripMenuItem.Visible = true;
			buttonReturnPositive.Visible = true;
			buttonReturnComparative.Visible = true;
			buttonReturnSuperlative.Visible = true;
		}
		
		void ВыбратьПоложительнуюСтепеньToolStripMenuItemClick(object sender, EventArgs e)
		{
			if(listView1.SelectedIndices.Count > 0){
				TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[1].Text.ToString();
				Close();
		...
		void ВыбратьСравнительнуюСтепеньToolStripMenuItemClick(object sender, EventArgs e)
		{
			if(listView1.SelectedIndices.Count > 0){
				TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[2].Text.ToString();
				Close();
		...
		void ВыбратьПревосходнуюСтепеньToolStripMenuItemClick(object sender, EventArgs e)
		{
			if(listView1.SelectedIndices.Count > 0){
				TextBoxReturnValue.Text += listView1.Items[listView1.SelectedIndices[0]].SubItems[3].Text.ToString();
				Close();
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_ADJECTIVE;
		...
		void editFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				eForm.ID = listView1.Items[listView1.SelectedIndices[0]].SubItems[5].Text;
				eForm.DataType = Constants.DATA_TYPE_ADJECTIVE;
				eForm.ParentForm = this;
				eForm.Show();
			}
		...
		{
			if(listView1.SelectedIndices.Count > 0){
				String fileID = listView1.Items[listView1.SelectedIndices[0]].SubItems[5].Text;
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM Adjective WHERE (ID = " + fileID + ")");
					if(query.Execute()){
