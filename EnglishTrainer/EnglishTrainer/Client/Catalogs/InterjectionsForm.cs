...
{
	/// <summary>
	/// Description of InterjectionsForm.
	/// </summary>
	public partial class InterjectionsForm : Form
	{
		public InterjectionsForm()
		{
			//
		...
		void InterjectionsFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Interjections ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Interjections")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["Interjection_English"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["Interjection_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "Interjections";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Interjections WHERE (Interjection_Translation LIKE '%" + toolStripComboBox1.Text + 
				"%' OR Interjection_English LIKE '%" + toolStripComboBox1.Text + "%' ) ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Interjections")){
				listView1.Items.Clear();
				ListViewItem ListViewItem_add;
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_INTERJECTIONS;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM Interjections WHERE (ID = " + fileID + ")");
					if(query.Execute()){
