...
namespace EnglishTrainer.Client.Catalogs
{
	...
	public partial class ArticlesForm : Form
	{
		...
		void ArticlesFormLoad(object sender, EventArgs e)
		{
			TableRefresh();
		}
		...
		void TableRefreshLocal()
		{
			...
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Articles ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Articles")){
				...
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
	    		{
					...
					ListViewItem_add.SubItems.Add(rowElement["Article_English"].ToString());
					ListViewItem_add.StateImageIndex = 1;
					ListViewItem_add.SubItems.Add(rowElement["Article_Translation"].ToString());
		...
		void searchLocal()
		{
			...
			oleDb.dataSet.Clear();
			oleDb.dataSet.DataSetName = "Articles";
			oleDb.oleDbCommandSelect.CommandText = "SELECT * FROM Articles WHERE (Article_Translation LIKE '%" + toolStripComboBox1.Text + 
				"%' OR Article_English LIKE '%" + toolStripComboBox1.Text + "%' ) ORDER BY ID ASC";
			if(oleDb.ExecuteFill("Articles")){
				listView1.Items.Clear();
				ListViewItem ListViewItem_add;
				foreach(DataRow rowElement in oleDb.dataSet.Tables[0].Rows)
		...
		void addFile()
		{
			...
			eForm.DataType = Constants.DATA_TYPE_ARTICLES;
		...
		void deleteFile()
		{
			if(listView1.SelectedIndices.Count > 0){
				...
				if(MessageBox.Show("Удалить безвозвратно запись '" + fileName + "' ?"  ,"Вопрос:", MessageBoxButtons.YesNo) == DialogResult.Yes){
					...
					query.SetCommand("DELETE FROM Articles WHERE (ID = " + fileID + ")");
					if(query.Execute()){
